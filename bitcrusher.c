...
uint32_t bitcrushing_mask=-1 << 1;                                      // 1 bit crushed by default.
...
//define SPI bus configuration
    ...
    uint8_t mosi[10] =                                                  //12 bit ADC read 0x08 ch0, - 0c for ch1
    {
        0x01, 0x00, 0x00
    };
    uint8_t miso[10] = { 0 };
...
//update booster_value when the PUSH1 or 2 buttons are pushed.
            ...
            {                                                           //100ms delay for buttons debouncing
                bcm2835_delay(100);
                if (bitcrushing_value<12) {
                    bitcrushing_value++;
                    bitcrushing_mask = -1 << bitcrushing_value;
//                    printf ("val: %d mask: %x\n", bitcrushing_value , bitcrushing_mask);
                }
            }
            else if (PUSH1_val==0)
            {                                                           //100ms delay for buttons debouncing.
                bcm2835_delay(100);
                if (bitcrushing_value>0) {
                    bitcrushing_value--;
                    bitcrushing_mask = -1 << bitcrushing_value;
//                    printf ("val: %d mask: %x\n", bitcrushing_value , bitcrushing_mask);
                }
...
//The number of bits shifted are defined by bitcrushing_value
        output_signal = input_signal & bitcrushing_mask;
